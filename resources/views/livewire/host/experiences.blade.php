                        ...
                        @if($experience->available_dates)
                            <div class="small">
                                @foreach($experience->available_dates as $date)
                                    <span class="badge bg-info me-1">{{ \Carbon\Carbon::parse($date)->format('M d, Y') }}</span>
                                @endforeach
                            </div>
                        @endif
                    </td>
                ...
                <div class="modal-dialog modal-dialog-scrollable" style="margin: 1.75rem auto;">
                    <div class="modal-content">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    @if($photo || $existingPhoto)
                                        <button class="btn btn-outline-secondary" type="button" wire:click="$set('photo', null)">Clear</button>
                                    @endif
                                </div>
                                ...
                                @if ($photo && !$errors->has('photo'))
                                    <div class="mt-2">
                                        <img src="{{ $photo->temporaryUrl() }}" class="img-thumbnail" style="max-width: 200px; height: auto; object-fit: cover;">
                                    </div>
                                @elseif ($experience_id && $existingPhoto)
                                    <div class="mt-2">
                                        <img src="{{ Storage::url($existingPhoto) }}" class="img-thumbnail" style="max-width: 200px; height: auto; object-fit: cover;">
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    <input
                                        type="text"
                                        id="available_dates"
                                        wire:model="available_dates_raw"
                                        x-data
                                        x-init="
                                ...
                                <div class="form-text">Click to select multiple dates. Selected dates will appear here.</div>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" wire:click="$set('modalMode', false)">Close</button>
                            <button type="button" class="btn btn-primary" wire:click="{{ $experience_id ? 'update' : 'store' }}">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            Livewire.on('updateFlatpickrDates', ({ dates }) => {
